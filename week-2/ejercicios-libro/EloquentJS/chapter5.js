...
var flatted = arrays.reduce(function(prev, next){
...
var AgesWhenBorn = ancestry.map(function(child){
	if(child.mother !== null && child.mother !== undefined){
...
AgesWhenBorn = AgesWhenBorn.filter(function(age){
	return !!age;

});

console.log("Average age of mother when child is born: ",average(AgesWhenBorn));
// → 31.2

//Exercice 3 Historical life expectancy
...
ancestry.forEach(function(person){
	var index = Math.ceil(person.died/100);
	if(byCentury[index] === undefined){
		byCentury[index] = [];
	}
	byCentury[index].push(age(person.born, person.died));
});
